...
import numpy as np
import glob
from utils.scene_grids import get_grid_cell_index, get_sub_grid_cell_index
...
class Model_LSTM_Scene_SoftFilter(nn.Module):
    def __init__(self, data_loader, args, train = False):
        super(Model_LSTM_Scene_SoftFilter, self).__init__()
    ...
    def sample(self, cur_frame):
        ...
        # forward 
        assert self.scene_states["scene_h0_list"][self.dataset_id].sum() != 0, "Error: Zero scene data"
        assert self.scene_states["scene_c0_list"][self.dataset_id].sum() != 0, "Error: Zero scene data"
        self.get_scene_states(xabs)                          # each ~ [num_layers,batch_size,rnn_size]
        mu1, mu2, log_sigma1, log_sigma2, rho, pi_logits = self.forward(xoff, xabs)
    ...
    def initialize_scene_data(self, data_loader, args):
        ...
        self.scene_states = {
            "scene_h0_list" : torch.zeros(args.max_datasets,args.num_grid_cells**2,args.num_layers, 1, args.rnn_size),
            "scene_c0_list" : torch.zeros(args.max_datasets,args.num_grid_cells**2,args.num_layers, 1, args.rnn_size)
        }
        if(self.use_cuda): 
            self.scene_states["scene_h0_list"]     = self.scene_states["scene_h0_list"].cuda()
            self.scene_states["scene_c0_list"]     = self.scene_states["scene_c0_list"].cuda()

        self.scene_info ={ 
            "scene_grid_list" : [], 
        }   
        for d in range(args.max_datasets):
            grid_list = [i for i in range(args.num_grid_cells**2)]
            self.scene_info["scene_grid_list"].append(grid_list)

    def get_scene_states(self, xabs):  
        """
        functions get scene_states (scene memories) for each current pedestrian
        """        
        scene_grid_h0 =  torch.zeros((self.num_layers, self.batch_size, self.rnn_size))
        scene_grid_c0 =  torch.zeros((self.num_layers, self.batch_size, self.rnn_size))  
        if(self.use_cuda):
        ...
        for pid in range(self.batch_size):
            ...
            scene_grid_h0[:,pid,:]  = self.scene_states["scene_h0_list"][self.dataset_id][gid]
            scene_grid_c0[:,pid,:]  = self.scene_states["scene_c0_list"][self.dataset_id][gid]

        self.scene_grid_h0 = scene_grid_h0.view(-1,self.batch_size,self.rnn_size)
        self.scene_grid_c0 = scene_grid_c0.view(-1,self.batch_size,self.rnn_size)
        if(self.use_cuda):
    ...
    def update_scene_states(self, xabs):
        ...
        for pid in range(self.batch_size):
            # For each ped in current frame
            gid =  get_grid_cell_index(xabs[pid].detach().numpy(), self.num_grid_cells) # current grid-cell
            self.scene_states["scene_h0_list"][self.dataset_id][gid] = self.scene_grid_h0[:,pid,:].data.clone()
            self.scene_states["scene_c0_list"][self.dataset_id][gid] = self.scene_grid_c0[:,pid,:].data.clone()
