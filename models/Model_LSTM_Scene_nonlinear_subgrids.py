...
class Model_LSTM_Scene_nonlinear_subgrids(nn.Module):
    ...
    def init_target_hidden_states(self):
       # Initialize states for all targets in current batch
        ...
        if(self.use_cuda): 
            self.i_h0, self.i_c0 = self.i_h0.cuda(), self.i_c0.cuda() 
    ...
    def get_onehot_location(self, x):
        ...
        for pedindex in range(self.batch_size):
            ...
            ped_location[0], ped_location[1] = x[pedindex, 0].cpu().detach().numpy(), x[pedindex, 1].cpu().detach().numpy()
    ...
    def get_scene_states(self, xabs):  
        ...
        for pid in range(self.batch_size):
            ...
            gid =  get_grid_cell_index(xabs[pid].cpu().detach().numpy(), self.num_grid_cells)        # current grid-cell
            sid =  get_sub_grid_cell_index(xabs[pid].cpu().detach().numpy(), self.num_grid_cells, self.num_sub_grids)        # current grid-cell
