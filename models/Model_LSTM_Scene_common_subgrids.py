...
import numpy as np
import glob
from utils.scene_grids import get_common_grids, get_grid_cell_index, get_sub_grid_cell_index
...
class Model_LSTM_Scene_common_subgrids(nn.Module):
    def __init__(self, data_loader, args, train = False):
        super(Model_LSTM_Scene_common_subgrids, self).__init__()
    ...
    def sample(self, cur_frame):
        ...
        # forward 
        assert self.scene_states["common_h0_list"][self.dataset_id].sum() != 0, "Error: Zero scene data"
        assert self.scene_states["common_c0_list"][self.dataset_id].sum() != 0, "Error: Zero scene data"
        self.get_scene_states(xabs)                          # each ~ [num_layers,batch_size,rnn_size]
        mu1, mu2, log_sigma1, log_sigma2, rho, pi_logits = self.forward(xoff, xabs)
    ...
    def initialize_scene_data(self, data_loader, args):
        ...
        if(self.use_cuda): 
            self.scene_states["common_h0_list"]     = self.scene_states["common_h0_list"].cuda()
            self.scene_states["common_c0_list"]     = self.scene_states["common_c0_list"].cuda()
        ...
        assert args.num_common_grids > 0,"num_common_grids is 0."       
        assert args.use_sub_grids_map is True,"use_sub_grids_map is False." 
        ...
        print(self.scene_info["common_sub_grids_maps"])
